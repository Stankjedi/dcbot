...
- **컨텍스트:** 글 제목/본문 일부 + (선택) 최근 댓글 일부 + (선택) 갤러리 검색 결과(“관련 글” 링크)
...
| 항목 | 평가 |
|------|------|
| 기능 완성도 | 검색 URL 생성(`src/lib/dcinside/url.ts`) + 결과 파싱(`src/lib/dcinside/search.ts`)으로 상위 N개 결과를 추출해 컨텍스트/답변 링크에 반영. 페이지에서 감지된 갤러리 컨텍스트가 있으면 이를 우선 사용. |
...
| 강점 | “관련 글” 링크로 답변 신뢰도/탐색성을 강화하며, 토큰 절감을 위해 검색을 필요 시에만 수행. |
