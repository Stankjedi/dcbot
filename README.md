...
- 답변 포맷: 댓글 답변은 항상 `디시봇:`으로 시작
- 검색(옵션): `@디시봇 검색:` 명령으로 갤러리 내부 검색(제목만 출력, URL 미포함)
- 링크 정책: 포털 검색 링크(URL) 1개까지 허용(그 외 URL 자동 제거)
...
- `@디시봇 요약` / `@디시봇 summary` : 현재 글 요약(제목/본문/댓글 기반, 가능한 경우)
- `@디시봇 설정` : 옵션 열기 안내
...
- 간단 커스터마이징: 옵션의 `인스트럭션(QA)`에서 “기본 지침/추가 지침”을 수정
- 요약 커스터마이징: 옵션의 `인스트럭션(요약)`에서 “기본 지침”을 수정
- 고급 커스터마이징:
