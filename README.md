...
- RAG-lite(옵션): 질문 키워드로 갤러리 내부 검색 → 관련 글 링크를 답변에 포함
...
- **갤러리 검색 대상**: 기본 `thesingularity`(마이너갤)  
  내부 검색(RAG-lite)에서 “관련 글”을 찾을 때 사용하는 갤러리 ID/종류입니다. 다른 갤에서 쓰면 여기 값을 바꾸세요.
- **최대 답변 글자수**: 최대 400 (디시 제한)
- **자동 답변 생성**: 기본 ON  
